...
class DBClient{
    ...
    init(dbid, host, user, password, database, callback) {
        ...
        console.log(util.format('mysql(%s) init %s %s@%s password is %s', dbid, database, user, host, password));
    ...
    connect(funcOnConnect) {
        ...
        if (dbclient.stateConnect == DBCONNECT_NO) {
            ...
            console.log(util.format('mysql connect %j', dbclient.cfg));
            ...
            dbclient.dbcon.connect(function (err) {
                if (err) {
                    ...
                    console.log(util.format('mysql connect error %j', err));
    ...
    onDBError(err) {
        ...
        if (err) {
            ...
            if (dbclient.curSQL != undefined) {
                console.log(util.format('error query sql is %s', dbclient.curSQL));
            }
        ...
        dbclient.connect(function () {
            ...
            dbclient.dbcon.query(sql, function (err, rows, fields) {
                if (err) {
                    ...
                    console.log(util.format('DBClient query(%s) err is %j', sql, err));
                }
    ...
    _queryList(sqlarr, begin, max, result, func) {
        ...
        dbclient.dbcon.query(sqlarr[begin], function (err, rows, fields) {
            if (err) {
                ...
                console.log(util.format('DBClient _queryList(%s) err is %j', dbclient.curSQL, err));
            }
