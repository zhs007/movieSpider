...
class RotateUserAgentMiddleware(UserAgentMiddleware):
    ...
    def process_request(self, request, spider):  
        ua = self.user_agent_list[self.curindex]#random.choice(self.user_agent_list)
        self.curindex = self.curindex + 1
        if self.curindex >= len(self.user_agent_list):
            self.curindex = 0
        ...
        if ua:  
            ...
            request.headers.setdefault('User-Agent', ua)#'Mozilla/5.0 (Macintosh; Intel Mac OS X 10_11_1) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/47.0.2526.73 Safari/537.36')
